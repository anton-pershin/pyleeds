...
enum BaseFlow        {Zero, PlaneCouette, Parabolic}; /// enum BaseFlow        {Zero, PlaneCouette, Parabolic, ASBL};
...
public:
  ...
  DNSFlags(BaseFlow        baseflow       = Zero,
	   ...
	   Real            dPdx           = 0.0,
	   Real            Ubulk          = 0.0,
///      Real            nu             = 0.0, // TobiasHack
///      Real            Usuck          = 1.0, // TobiasHack
...
///   Real            nu;           // TobiasHack - viscosity = delta/Re
///   Real            Usuck;        // TobiasHack - suction velocity
...
/// void  navierstokesNL(const FlowField& u, const FlowField& ubase,
///          const ChebyCoeff& Ubase, FlowField& f, FlowField& tmp,
///          FlowField& tmp2, const DNSFlags& flags);
/// 
///          
/// void navierstokesNL(const FlowField& u_, const ChebyCoeff& Ubase,
///         FlowField& f, FlowField& tmp, const DNSFlags& flags);
///         
/// ChebyCoeff ConstructBaseFlow(const DNSFlags& flags, int Ny, Real a, Real b, Real ua = -0.5, Real ub = 0.5); /// Construct ChebyCoeff from baseflow flag
/// 
/// /// End TobiasHack
...
public:
  ...
  DNS(const FlowField& u, const ChebyCoeff& Ubase,
      Real nu, Real dt, const DNSFlags& flags, Real t=0.0);
      
///   DNS(const FlowField& u, Real dt, const DNSFlags& flags, Real t=0.0); // TobiasHack
